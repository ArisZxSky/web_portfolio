(function ($) {
    ...
    $(".testimonial-carousel").owlCarousel({
        ...
        // Lightbox untuk gambar sertifikat
    $('.certificate-enlarge').on('click', function() {
        var fullSrc = $(this).data('full-src');
        var lightboxOverlay = $('#lightboxOverlay');
        var lightboxImage = $('#lightboxImage');
        lightboxImage.attr('src', fullSrc);
        lightboxOverlay.css('display', 'flex'); // Tampilkan overlay
        // Tambahkan kelas untuk transisi zoom
        setTimeout(function() {
            lightboxImage.css('transform', 'scale(1)'); // Zoom in
        }, 10); // Sedikit delay agar transisi terlihat
    });
    // Tutup lightbox saat overlay diklik
    $('#lightboxOverlay').on('click', function() {
        var lightboxImage = $('#lightboxImage');
        lightboxImage.css('transform', 'scale(0.8)'); // Zoom out
        // Sembunyikan overlay setelah transisi selesai
        setTimeout(function() {
            $('#lightboxOverlay').css('display', 'none');
            lightboxImage.attr('src', ''); // Kosongkan src gambar
